...
const CreateTeam = () => {
  ...
  const isValidTeam = () => {
    ...
    // Check credits limit
    if (getTotalCreditsUsed() > 100) return false
    
    const composition = getTeamComposition()
    
    // At least 1 wicket-keeper
    if (composition['wicket-keeper'] < 1) return false
    
    // At least 3 batsmen
    if (composition.batsman < 3) return false
    
    // At least 3 bowlers
    if (composition.bowler < 3) return false
    
    // Players from both teams
    if (composition.team1 < 1 || composition.team2 < 1) return false
          ...
          <button
            ...
            onClick={() => {
              if (isValidTeam()) {
                ...
                console.log('Next clicked - team is valid')
              }
            }}
          >
            NEXT
