const PlayerScoreSchema = new mongoose.Schema({
  ...
  batting: {
    runs: { 
      type: Number, 
      default: function() { return this.sport === 'cricket' ? 0 : undefined; }
    },
    ballsFaced: { 
      type: Number, 
      default: function() { return this.sport === 'cricket' ? 0 : undefined; }
    ...
    isOut: { 
      type: Boolean, 
      default: function() { return this.sport === 'cricket' ? false : undefined; }
    },
    howOut: {
      type: String,
      enum: ["bowled", "caught", "lbw", "stumped", "run out", "hit wicket", "not out", "retired hurt"],
      validate: {
        validator: function() {
  ...
  football: {
    goals: { 
      type: Number, 
      default: function() { return this.sport === 'football' ? 0 : undefined; }
    },
    assists: { 
      type: Number, 
      default: function() { return this.sport === 'football' ? 0 : undefined; }
    ...
    cleanSheet: { 
      type: Boolean, 
      default: function() { return this.sport === 'football' ? false : undefined; }
    },
    minutesPlayed: { 
      type: Number, 
      default: function() { return this.sport === 'football' ? 0 : undefined; }
    ...
    position: {
      ...
      validate: {
        validator: function() {
          return this.sport === 'football' || this.football?.position === undefined;
        },
  ...
  kabaddi: {
    raiding: {
      totalRaids: { 
        type: Number, 
        default: function() { return this.sport === 'kabaddi' ? 0 : undefined; }
      },
      successfulRaids: { 
        type: Number, 
        default: function() { return this.sport === 'kabaddi' ? 0 : undefined; }
    ...
    position: {
      ...
      validate: {
        validator: function() {
          return this.sport === 'kabaddi' || this.kabaddi?.position === undefined;
        },
...
PlayerScoreSchema.methods.calculateFantasyPoints = function() {
  ...
  switch(this.sport) {
    case 'cricket':
      ...
      this.fantasyPoints.battingPoints = (this.batting?.runs || 0) + (this.batting?.fours || 0) + (this.batting?.sixes || 0) * 2;
    ...
    case 'football':
      ...
      this.fantasyPoints.attackingPoints = (this.football?.goals || 0) * 16 + (this.football?.assists || 0) * 12;
      this.fantasyPoints.penaltyPoints = -((this.football?.yellowCards || 0) + (this.football?.redCards || 0) * 3);
      break;
    ...
    case 'kabaddi':
      ...
      this.fantasyPoints.attackingPoints = (this.kabaddi?.raiding?.raidPoints || 0) + (this.kabaddi?.raiding?.superRaids || 0) * 5;
      this.fantasyPoints.defendingPoints = (this.kabaddi?.defending?.tacklePoints || 0);
      break;
  }
