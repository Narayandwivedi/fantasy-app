...
const Sidebar = memo(({ collapsed, onToggle }) => {
  ...
  return (
    <div className={`relative lg:h-screen ${collapsed ? 'lg:max-w-[80px]' : 'lg:max-w-[320px]'} bg-gradient-to-b from-slate-900 via-purple-900 to-slate-900 shadow-2xl border-r border-purple-500/20 backdrop-blur-sm transition-all duration-300 flex flex-col`}>
      ...
      <button
        onClick={onToggle}
        className="lg:hidden absolute top-4 right-4 z-10 bg-gradient-to-r from-purple-500 to-blue-500 text-white p-2 rounded-full shadow-lg hover:scale-110 transition-all duration-200 hover:shadow-xl"
      ...
      {/* Header */}
      <div className='relative hidden lg:block p-6 border-b border-purple-400/30 flex-shrink-0'>
      ...
      {/* Navigation */}
      <nav className='relative p-6 lg:flex-1 lg:overflow-y-auto'>
        {/* Expanded View */}
        <div className={`${collapsed ? 'hidden' : 'grid grid-cols-2 gap-3 lg:flex lg:flex-col lg:space-y-3'}`}>
          {navItems.map((item) => {
              ...
              <Link key={item.path} to={item.path.trim()}>
                <div className={getNavItemStyles(item.path)}>
                  ...
                  <div className={`relative ${active ? 'scale-110' : 'group-hover:scale-110'} transition-transform duration-200`}>
                    <IconComponent className={`w-5 h-5 mb-1 lg:mb-0 lg:mr-3 ${
        ...
        <div className={`${collapsed ? 'grid grid-cols-4 gap-3 lg:flex lg:flex-col lg:space-y-3' : 'hidden'}`}>
          {navItems.map((item) => {
            const IconComponent = item.icon;
            const active = isActive(item.path);
            ...
            return (
              <Link key={item.path} to={item.path.trim()}>
                <div className={`group relative flex items-center justify-center w-12 h-12 lg:w-full lg:px-4 lg:py-3 rounded-xl transition-all duration-300 transform hover:scale-105 ${
                  active
                    ? 'bg-gradient-to-r from-blue-500 to-purple-600 text-white shadow-lg shadow-blue-500/30 border border-blue-400/50'
                    : 'bg-white/10 backdrop-blur-sm hover:bg-gradient-to-r hover:from-purple-500/80 hover:to-blue-500/80 text-white/90 hover:text-white border border-white/20 hover:border-purple-400/50 hover:shadow-lg hover:shadow-purple-500/25'
                }`} title={item.label}>
      ...
      <div className={`p-3 lg:p-6 border-t border-purple-400/30 bg-gradient-to-t from-slate-900/90 to-slate-900/70 backdrop-blur-md lg:mt-auto lg:absolute lg:bottom-0 lg:left-0 lg:right-0 ${collapsed ? 'hidden lg:block' : ''}`}>
        {!collapsed && adminUser && (
        ...
        <button
          ...
          className={`group relative flex flex-col lg:flex-row items-center ${collapsed ? 'justify-center' : 'justify-center lg:justify-start'} w-full px-1.5 py-1.5 lg:px-4 lg:py-3 rounded-lg transition-all duration-300 transform hover:scale-105 bg-red-500/20 backdrop-blur-sm hover:bg-red-500/30 text-white/90 hover:text-white border border-red-400/30 hover:border-red-400/50 hover:shadow-lg hover:shadow-red-500/25`}
          title="Logout"
        >
          <div className="relative group-hover:scale-110 transition-transform duration-200">
            <LogOut className={`w-3.5 h-3.5 lg:w-5 lg:h-5 ${collapsed ? '' : 'mb-0.5 lg:mb-0 lg:mr-3'} text-white/80 group-hover:text-white`} />
          </div>
          
          {!collapsed && (
            <span className="text-[10px] lg:text-sm font-semibold transition-all duration-200 text-white/80 group-hover:text-white">
              Logout
            </span>
          )}

          <div className="absolute inset-0 bg-gradient-to-r from-white/0 to-white/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-lg"></div>
        </button>
      </div>
