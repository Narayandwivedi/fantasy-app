...
const EditBlogModal = ({ blog, onClose, onBlogUpdated }) => {
  ...
  useEffect(() => {
    if (blog) {
      setFormData({
        ...
        category: blog.category || 'fantasy-tips',
        tags: blog.tags ? blog.tags.join(', ') : '',
        author: blog.author || 'Winners11 Team',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${backendUrl}/api/blog/${blog._id}`, {
        method: 'PUT',
        credentials: 'include',
        body: submitFormData
      });
      ...
      <div className="bg-gradient-to-br from-slate-900 to-purple-900 rounded-2xl border border-purple-500/30 shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b border-purple-500/30">
          <div>
            ...
            <p className="text-purple-300 mt-1">Update your blog post</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                {currentImage && (
                  <div className="mb-3">
                    <img 
                      src={`${backendUrl}/images/blog/${currentImage}`}
                      alt="Current featured image"
                      className="w-full h-32 object-cover rounded-lg border border-white/20"
                    />
                    <p className="text-white/60 text-sm mt-1">Current image</p>
                  </div>
                )}
            ...
            <div className="space-y-6">
              ...
              {/* Blog Stats */}
              <div className="bg-white/10 rounded-xl p-4 border border-white/20">
                <h3 className="text-white font-semibold mb-2">Blog Stats</h3>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-white/60">Views:</span>
                    <span className="text-white ml-2">{blog.views || 0}</span>
                  </div>
                  <div>
